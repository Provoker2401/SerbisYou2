...
const ElectronicApplianceCleaning = ({ route }) => {
  ...
  const [kitchenPrice, setkitchenPrice] = useState(null);
  const [ovenPrice, setovenPrice] = useState(null);
  const [refriPrice, setrefriPrice] = useState(null);
  const [tvPrice, settvPrice] = useState(null);
  const [washingPrice, setwashingPrice] = useState(null);
  ...
  useEffect(() => {
    ...
    const subCollectionRef = doc(
      ...
      documentName,
      subCollectionName,
      "electronicAppCleaning"
    );

    getDoc(subCollectionRef)
      .then((docSnapshot) => {
        if (docSnapshot.exists()) {
          const electronData = docSnapshot.data();

          setareaPrice(electronData.area);
          setfanPrice(electronData.fan);
          setkitchenPrice(electronData.kitchen);
          setovenPrice(electronData.oven);
          setrefriPrice(electronData.refri);
          settvPrice(electronData.tv);
          setwashingPrice(electronData.washing);
        } else {
  ...
  const multipliedValue =
    parseInt(input1Value) * areaPrice +
    parseInt(input2Value) * refriPrice +
    parseInt(input3Value) * fanPrice +
  ...
  const servicePrices = {
    ...
    washing: washingPrice,
    oven: ovenPrice,
  };

  const inputValues = [
    { name: "Area Conditioner", value: input1Value, service: "area" },
    { name: "Refrigerator", value: input2Value, service: "refri" },
    { name: "Electric Fan", value: input3Value, service: "refri" },
    { name: "Television", value: input4Value, service: "tv" },
    { name: "Kitchen Stove", value: input5Value, service: "kitchen" },
    { name: "Washing Machine", value: input6Value, service: "washing" },
    { name: "Microwave Oven", value: input7Value, service: "oven" },
  ];
  ...
  const openModalWithData = () => {
    ...
    if (filteredInputsWithTotalPrice.length > 0) {
      ...
      setReviewData({
        ...
        category: "Electronic Appliance Cleaning", // Add the string here
        logo: "mask-group16.png",
        title: "Cleaning",
      });
    } else {
  ...
  return (
    <View style={styles.electronicApplianceCleaning}>
      ...
      >
        <View
          style={[
            styles.electronicApplianceCleaningWrapper,
            styles.btnWrapperFlexBox,
          ]}
        >
          <Text style={styles.electronicApplianceCleaning1}>
        ...
        <View style={styles.componentspropertyTypeParent}>
          ...
          <View style={[styles.frameGroup, styles.frameParentSpaceBlock]}>
            ...
            <View style={styles.frameContainer}>
              ...
              >
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Area Conditioner 
`}</Text>
                    <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading ? "Loading..." : `₱${areaPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible1 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Refrigerator 
`}</Text>
                    <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading ? "Loading..." : `₱${refriPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible2 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Electric Fan
`}</Text>
                    <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading ? "Loading..." : `₱${fanPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible3 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Television 
`}</Text>
                    <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading ? "Loading..." : `₱${tvPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible4 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Kitchen Stove
`}</Text>
                      <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading
                        ? "Loading..."
                        : `₱${kitchenPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible5 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Washing Machine
`}</Text>
                      <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading
                        ? "Loading..."
                        : `₱${washingPrice} per unit`}
                    </Text>
                  </Text>
                </View>
                {areaVisible6 ? (
              ...
              <View style={[styles.frameParent1, styles.frameParentSpaceBlock]}>
                <View style={styles.areaConditioner500PerUnitWrapper}>
                  <Text style={styles.areaConditioner500Container}>
                    <Text style={styles.areaConditioner}>{`Microwave Oven 
`}</Text>
                       <Text style={[styles.perUnit, styles.perUnitTypo]}>
                      {loading
                        ? "Loading..."
                ...
                {areaVisible7 ? (
                  <View>
                    <AddMinusStepper
                      ...
                      onDecrement={() => {
                        setInput7Value(input6Value - 1);
                        if (input6Value === 1) setInput7Value(input7Value - 1);
                      }}
...
const styles = StyleSheet.create({
  ...
  electronicApplianceCleaningWrapper: {
    alignItems: "center",
    alignSelf: "stretch",
  },
  tag: {
  ...
  areaConditioner500Container: {
    color: Color.colorTypographyContentIconsBlack,
    textAlign: "left",
    flex: 1,
  },
  areaConditioner500PerUnitWrapper: {
    flexDirection: "row",
    flex: 1,
  },
  add2Child: {
...
export default ElectronicApplianceCleaning;
